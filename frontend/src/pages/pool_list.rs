...
pub fn PoolListPage() -> impl IntoView {
    ...
    let pools = LocalResource::new(move || {
        ...
        async move {
            loading.start();
            let result = client.get_pools(&p.query, p.offset, p.limit, FIELDS).await;
            loading.finish();
            result
        }
    });
    ...
    let accumulated = RwSignal::new(Vec::<PoolInfo>::new());
                                ...
                                view! {
                                    <table class="pool-list">
                                        ...
                                        <tbody>
                                            <For
                                                ...
                                                key=|pool| pool.id.unwrap_or(0)
                                                children=move |pool| {
                                                    render_pool_row(pool)
                                                }
                                            />
                                        </tbody>
                                    </table>
                                    <div class="scroll-sentinel">
                                        {move || loading_more.get().then(|| view! {
                                            <p class="loading-more">"Loading more pools..."</p>
                                        })}
                                        {move || {
